...
## 29. 2026-04-29 mode7 primary PDF extraction continuation

### 29.1 today's focus
- continued the new `mode7` pipeline for primary-school written-paper extraction
- worked around the current absence of junior-high PDFs and used the existing PEP primary PDF bank as the main validation set
- kept the agreed output rule:
  - root: `tool/exam_data/教材版本/task_时间戳/`
  - this round full-run output: `tool/exam_data/人教版（PEP）小学/task_20260429_002302/`

### 29.2 metadata / output conventions confirmed
- `tool/mode7_primary_pdf_extract.py`
  - output naming keeps grade / semester / optional unit / source pdf stem
  - `unit` is optional so monthly / midterm / final papers without unit can still land correctly
  - current primary output stays under the textbook-version folder only, without repeating the long version string in each filename

### 29.3 key bug fixed today
- located a real regression in `find_answer_start_index()`
- previous behavior treated any line containing `答案` as the beginning of the answer section
- this incorrectly matched normal question headings like `八、选择最佳答案`
- result: later written sections were cut off before parsing, which caused severe under-extraction in some papers
- fix applied:
  - tightened the rule to only match real answer-page heading patterns such as:
    - `参考答案`
    - `答案`
    - `听力原文及参考答案`
    - `听力原文`
    - `听力材料`
  - no longer cuts just because a normal stem contains the word `答案`

### 29.4 targeted verification after the fix
- verified the regression directly on:
  - `三上U1.pdf`
  - `三上U2.pdf`
- recovery result:
  - `三上U1` restored from `record_count = 0` to `record_count = 4`
  - `三上U2` restored from `record_count = 0` to `record_count = 4`
- confirmed that later sections such as `8-12` were no longer being truncated out of the written part

### 29.5 full rerun completed
- ran the full primary PEP PDF set again with 5-key concurrency
- command shape used:
  - `conda run --no-capture-output -n english_book python tool/mode7_primary_pdf_extract.py --input-dir "tool/题库/人教版（PEP）小学" --output-dir "tool/exam_data" --max-workers 5`
- full-run summary:
  - file count: `68`
  - run tag: `task_20260429_002302`

### 29.6 current post-rerun status
- reliable summary from the new batch:
  - total reports: `68`
  - `record_count = 0`: `4`
  - `record_count <= 2`: `6`
  - duplicate-stop warnings: `32`
  - skipped `unsupported` sections: `127`
- important interpretation:
  - the four `0-record` files are currently all composition-oriented primary files:
    - `五上作文`
    - `五下作文`
    - `六上作文`
    - `六下作文(2)`
  - this is consistent with the later decision that primary composition / writing-expression content does not need to be included

### 29.7 remaining problem classes
- duplicate-section stop logic is still too aggressive in many files
  - many reports still contain warnings like `duplicate_section_sequence_stopped_at_*`
  - this means the current anti-answer-page cutoff still needs another pass
- unsupported primary item types remain the main gap
  - examples include:
    - image-label / image-word correspondence variants
    - some low-grade image tasks
    - some situational-choice variants
    - some task-reading variants
    - some phonics / ordering / word-bank variants
- a few partially detected papers still have low final `record_count` because later item-type parsers are not finished yet

### 29.8 files most relevant to today
- `tool/mode7_primary_pdf_extract.py`
- `CODEX_LOG.md`
- output batch:
  - `tool/exam_data/人教版（PEP）小学/task_20260429_002302/`

### 29.9 suggested next step for the next session
- first:
  - formally filter out primary composition / writing PDFs at the input stage
- then:
  - refine the duplicate-section stop strategy so it blocks answer pages without prematurely cutting valid later questions
- after that:
  - continue filling the highest-frequency `unsupported` primary question types
