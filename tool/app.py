...
任务：识别图片中的英语单词表和短语表，按照从上到下的视觉顺序提取为 JSON 数组。

布局说明：
1. 图片已预处理为单列布局，请按从上到下的顺序依次识别所有内容
2. 单元标识格式为 "Unit" + 数字，如 "Unit 1", "Unit 2" 等，当识别到新的单元标识时，后续所有单词/短语都属于该新单元

字段要求：
- word：英文单词或短语
- phonetic：音标，必须是标准格式，用斜杠包围，如 /ˈwɔːtər/
- meanings：数组，每个元素包含 pos, meaning, example, example_zh
- unit：所属单元，格式：Unit X
- type：word 或 phrase

格式规范：
1. phonetic 必须是标准音标格式，用斜杠包围，如 /ˈeksəmpəl/，不要包含词性或其他内容
2. pos 必须使用带点的缩写形式：n. v. adj. adv. prep. pron. conj. interj. num. art. phr. 等
3. meaning 使用中文释义
4. example 使用英文例句，example_zh 使用对应的中文翻译
5. 如果同一个单词有多个词性，必须合并为一条记录，meanings 数组包含多个元素
6. 请直接输出 JSON 数组，不要输出 Markdown 代码块标记
"""
    request_kwargs = {
        "model": model_name,
        "messages": [
...
def complete_word_info(word, meaning, api_key, base_url, model_name, grade_level, item_type="word"):
    ...
    type_desc = "短语" if item_type == "phrase" else "单词"

    prompt = f"""
你是一个专业的英语教材教研专家，正在补充{type_desc}的详细信息。
...
格式要求：
1. phonetic 必须是标准音标格式，用斜杠包围，如 /ˈwɔːtər/，不要包含其他内容
2. pos 必须使用带点的缩写形式：n. v. adj. adv. prep. pron. conj. interj. num. art. phr. 等
3. meaning 使用中文释义
4. example 使用英文例句，example_zh 使用对应的中文翻译
5. 例句要适合该年级水平，简洁易懂
6. 请直接输出 JSON 对象，不要输出 Markdown 代码块标记

示例输出格式：
{{"phonetic": "/ˈeksəmpəl/", "meanings": [{{"pos": "n.", "meaning": "例子", "example": "This is an example.", "example_zh": "这是一个例子。"}}]}}
"""
    request_kwargs = {
        "model": model_name,
    ...
    if error:
        st.warning(f"信息补全失败，已重试 {LLM_MAX_RETRIES} 次: {error}")
        return None
